    ...
    for subdir, dirnames, filenames in os.walk(config.CATCHED_RPI_DIRECTORY):
        for f in os.listdir(subdir):
            ...
            for c in content_tmp:
                ...
                var_infos = []
                # RPI Entry carries addtional information besides key and timestamp

                oss = 'unknown'
                rssi = []
                for var_info in c_tmp[2:]:
                    if isinstance(var_info, str) and (var_info.lower() == 'android' or var_info.lower() == 'ios'):
                        oss = var_info
                ...
                if key_val in res:
                    ...
                    timedelta = base_date_time + d.timedelta(seconds=int(c_tmp[1]))
                     #                key, [time count,(RSSI)], id set, aem, os, duplicate_counter
                    var_info = [timedelta.timestamp()] + rssi                      
                    
                    res[key_val] = [key_val,[var_info], f, key_tmp[16:],oss,1]
